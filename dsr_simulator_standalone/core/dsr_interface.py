"""
DSR Interface - Distribution System Restoration Simulation Main Interface
==========================================================================

This module is the simulator's core external interface, orchestrating all operations.
Users (or LLM) interact with the simulation environment through this interface.

Main API:
---------
Initialization:
  - setup_case(case_name)       : Set up training scenario
  - reset(episode_config)       : Reset environment
  - create_network(nodes, edges): Create network topology
  - add_repair_crew()           : Add repair crew
  - add_mobile_power()          : Add mobile power source
  - add_fault()                 : Add fault

RC Operations:
  - move_repair_crew(rc_id, target) : Move RC (supports DP auto-conversion to node position)
  - repair_fault(rc_id, fault_id)   : Execute repair

MPS Operations:
  - move_mps(mps_id, target)                 : Move MPS
  - set_mps_output(mps_id, P, Q, target_loads) : Set MPS output power and target loads
    * If MPS is not connected, automatically executes connect()
    * Output is projected by _project_mps_outputs() to match actual load demand

Switch Operations:
  - operate_switches(switch_ops) : Batch switch operations
    * Automatically detects loops; violating operations are rolled back

Time Advancement:
  - advance_time() : Advance one time step, return event list

State Query:
  - get_current_state() : Get complete system state dictionary

MPS Power Projection (_project_mps_outputs):
  When multiple MPS serve the same loads, actual demand is allocated proportionally
  based on requested power ratios. Ensures sum(P_MPS) = sum(P_load) (power balance).
"""
...
class IndependentDSRInterface:
    ...
    def get_current_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Build MPS power allocation map: island -> (total_mps_output, total_load_demand, loads_in_island)
            # This ensures proper proportional allocation of MPS capacity
            # Note: Use load.state=='on' to determine if load has power (state has dual meaning)
            mps_allocation_map = {}
            if hasattr(self.env.egraph, 'identify_islands'):
                islands = self.env.egraph.identify_islands()
                for island_idx, island in enumerate(islands):
                    # Find MPS output in this island
                    total_mps_output = 0.0
                    for node_id in island:
                        if node_id in self.env.egraph.nodes:
                            if hasattr(self.env.egraph.nodes[node_id], 'type') and self.env.egraph.nodes[node_id].type == NodeType.MPS:
                                mps = self.agent_manager.get_mobile_power_source(node_id)
                                if mps and mps.state == 'connected' and mps.energy > 0:
                                    total_mps_output += mps.Pout

                    # Find total load demand in this island (only state='on' loads)
                    loads_in_island = []
                    total_load_demand = 0.0
                    for load_id, load in self.agent_manager.loads.items():
                        if load.node_id in island and load.state == 'on':
                            loads_in_island.append(load_id)
                            total_load_demand += load.P

                    # Store allocation data for this island
                    for load_id in loads_in_island:
                        mps_allocation_map[load_id] = {
                            'total_mps_output': total_mps_output,
                            'total_load_demand': total_load_demand,
            ...
            loads = {}
            for load_id, load in self.agent_manager.loads.items():
                # Calculate served power for this load
                # Use P_retained (cumulative, monotonically increasing) as the primary source
                # P_retained is updated by update_system_state() and never decreases
                mps_served_kw = 0.0
                served_grid_kw = 0.0

                if load.state == 'on':
                    # Check if it's grid-powered
                    is_grid_powered = False
                    if hasattr(self.env.egraph, 'nodes'):
                        for node_id, node_data in self.env.egraph.nodes.items():
                            if hasattr(node_data, 'type') and node_data.type == NodeType.GRID:
                                if self._is_connected(load.node_id, node_id, self.env.egraph):
                                    is_grid_powered = True
                                    served_grid_kw = load.P
                                    break

                    if not is_grid_powered:
                        # Use P_retained: includes both current MPS supply and historical retained power
                        mps_served_kw = getattr(load, 'P_retained', 0.0) if load.state == 'on' else 0.0
                ...
                loads[load_id] = {
                    ...
                    'Q': load.Q,
                    'W': load.W,
                    'state': load.state,  # actual physical state: 'on' or 'off'
                    'currently_served': load.state == 'on',  # actual instantaneous serving
                    'restored': restored,  # monotonic flag: True once ever served (informational)
                    'served_grid_kw': served_grid_kw,  # Explicit grid power
                    'served_mps_kw': mps_served_kw  # Current MPS power
                }
            ...
            if hasattr(self.env, 'egraph') and hasattr(self.env.egraph, 'nodes'):
                for nid, node in self.env.egraph.nodes.items():
                    # Check if node has powered loads (state=='on')
                    has_powered_load = any(
                        load.node_id == nid and load.state == 'on'
                        for load in self.agent_manager.loads.values()
                    )
                    nodes[nid] = {
                        'type': node.type.name if hasattr(node.type, 'name') else str(node.type),
                        'has_powered_load': has_powered_load  # Whether node has loads with state='on'
                    }
            ...
            # Count loads serving customers (state=='on' means load is serving)
            # Note: state='on' ⟺ load has power AND is serving customers
            serving_count = sum(1 for load in self.agent_manager.loads.values() if load.state == 'on')
            ...
            # Calculate weighted restoration ratio for observation
            # Uses same logic as reward system: sum(W*P for serving) / sum(W*P for all)
            # This ensures Stage 2 detection aligns with r_serve and r_delta calculations
            total_weighted_demand = sum(
                load.W * load.P for load in self.agent_manager.loads.values()
            )
            ...
            return {
                ...
                'powered_loads': serving_count,  # Count of loads with state='on' (backward compatibility)
                ...
                'restoration_summary': {  # Add restoration summary for observation
                    'energized_loads': serving_count,  # Same as powered_loads (state='on' count)
                    'total_loads': len(loads),
                ...
                if load.node_id in target_island:
                    ...
                    if load.state == 'on':
                        if has_grid_in_island:
                            ...
                            # Load is on but not in any MPS target_loads - should not happen
                            # Treat as offline for safety
                            total_p_load += load.P
                            total_q_load += load.Q
                        continue
        ...
        try:
            ...
            for item in mps_outputs:
                ...
                if mps.target_loads:
                    ...
                    # MPS has no specified target_loads -> error
                    return mps_outputs, {
                        "projected": False,
            ...
            for load_id in all_target_loads:
                ...
                if load.node_id in target_island:
                    ...
                    if has_grid_in_island and load.state == 'on':
                        grid_served_loads.append(load_id)
                else:
                    unreachable_loads.append(load_id)
            ...
            for load_id in all_target_loads:
                ...
                load = self.agent_manager.get_load(load_id)
                if load and load.node_id in target_island:
                    # For MPS-served loads (state='on'), still count demand for power sharing
                    load_demands[load_id] = (load.P, load.Q)
            ...
            # ===== v5.8: Per-Load Power Sharing Calculation =====
            # For each load, count how many MPS are targeting it (including existing + new)
            # Then split that load's demand among those MPS

            load_mps_count = {}  # {load_id: number of MPS serving this load}
            for load_id in load_demands.keys():
                count = 0
                # Count new MPS
                for mps_id, tl_set in target_loads_by_mps.items():
                    if load_id in tl_set:
                        count += 1
                # Count existing MPS
                for existing_id, (existing_mps, existing_tl_set) in existing_mps_in_island.items():
                    if load_id in existing_tl_set:
                        count += 1
                load_mps_count[load_id] = max(count, 1)  # At least 1 to avoid division by zero

            # Build list of all MPS for renegotiation (existing + new)
            all_mps_in_island = []

            # Add existing MPS
            for existing_id, (existing_mps, _) in existing_mps_in_island.items():
                p_requested = getattr(existing_mps, 'P_requested', existing_mps.Pout)
                q_requested = getattr(existing_mps, 'Q_requested', existing_mps.Qout)
                all_mps_in_island.append((existing_id, p_requested, q_requested))

            # Add new MPS being set
            for item in mps_outputs:
                all_mps_in_island.append((item["mps_id"], item["P_out"], item["Q_out"]))

            # ===== v5.8: Per-MPS Projection based on individual target_loads =====
            # Each MPS is projected based on its OWN target_loads demand
            # If multiple MPS target the same load, they share that load's demand equally
            ...
            for item in mps_outputs:
                mps_id = item["mps_id"]
                p_requested = item["P_out"]
                q_requested = item["Q_out"]
                mps = mps_objects[mps_id]
                ...
                for load_id in my_target_loads:
                    if load_id in load_demands:
                        p_load, q_load = load_demands[load_id]
                        # Divide by number of MPS serving this load
                        num_serving = load_mps_count.get(load_id, 1)
                        my_p_demand += p_load / num_serving
                        my_q_demand += q_load / num_serving
                ...
                if node and node.type == NodeType.MPS:
                    ...
                    if existing_mps and existing_mps.state == "connected" and existing_mps.id not in mps_ids_being_set:
                        ...
                        for load_id in existing_target_loads:
                            if load_id in load_demands:
                                p_load, q_load = load_demands[load_id]
                                # Use load_mps_count which already includes both existing and new MPS
                                num_serving = load_mps_count.get(load_id, 1)
                                existing_p_demand += p_load / num_serving
                                existing_q_demand += q_load / num_serving
    ...
    def operate_switches(self, switch_operations: List[Dict[str, str]]) -> Dict[str, Any]:
        """Operate multiple switches atomically with cycle detection

        Behavior:
        - All switches are updated atomically
        - If resulting topology has a cycle, ALL operations are reverted
        - LLM must learn to avoid creating cycles
        ...
        Returns:
            Result dict with:
                - status: "success" or "invalid" or "error"
                - cycle_detected: bool
                - message: str
        ...
        try:
            ...
            # Step 2: Save initial states (ALL switches, not just operated ones)
            initial_states = {sid: s.state for sid, s in self.env.agent_manager.switches.items()}

            # Step 3: Apply all operations
            for switch, operation in validated_ops:
                switch.state = operation

            # Step 4: Check for cycles (will revert if cycle detected)
            cycle_info = self.env.update_switch_effects(
                allow_faulted=False, revert_on_cycle=True,
                original_states=initial_states
            )
            ...
            weighted_power = cycle_info.get('weighted_power', 0.0)
            cycle_details = cycle_info.get('cycle_details', None)

            if cycle_detected:
                # Cycle was created - INVALID action
                # Switches have already been reverted
                ...
                final_states_str = ', '.join([f"{sid}:{state}" for sid, state in final_states_dict.items()])

                # Build error message with cycle diagnostics (objective information only)
                message_parts = [
                    f"INVALID - CYCLE DETECTED: Operations [{operations_desc}] would create a network loop.",
                    f"All {len(switch_operations)} operations REVERTED. Current states: {final_states_str}"
                ]

                # Add cycle path if available
                if cycle_details:
                    cycle_path = cycle_details.get('cycle_path', [])
                    switches_in_cycle = cycle_details.get('switches_in_cycle', [])

                    if cycle_path:
                        # Show cycle path (limit to 10 nodes to save tokens)
                        display_path = cycle_path[:10]
                        path_str = ' -> '.join(display_path)
                        if len(cycle_path) > 10:
                            path_str += f" ...+{len(cycle_path)-10}"
                        message_parts.append(f"Cycle: {path_str}")

                    if switches_in_cycle:
                        switches_str = ', '.join(switches_in_cycle[:5])
                        message_parts.append(f"Switches in cycle: {switches_str}")
    ...
    def _execute_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # MPS actions
            ...
            # v5.1+: MPS power control action with target_loads
