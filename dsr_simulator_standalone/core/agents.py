"""
Agent Classes - Agent Entity Classes
======================================

This module defines the 5 core agent entities in the DSR simulation:

1. Switch
   - Controls electrical edge connectivity/disconnection for network reconfiguration
   - State: open (disconnected) / close (closed)
   - Constraint: Closing a switch must not form a loop (distribution network must maintain radial topology)

2. RC (Repair Crew)
   - Moves on the traffic graph to fault points to perform repairs
   - State: idle -> moving -> idle -> repairing -> idle/out_of_service
   - Movement consumes time (distance/speed), repair consumes resources (efficiency p.u./step)
   - auto_repair=True enables automatic continuous repair until completion

3. Load
   - Represents customer power demand, attributes: P (active kW), Q (reactive kVar), W (priority weight)
   - State is automatically managed by the system: powered -> on, unpowered -> off
   - P_retained: Current actual power supply received (may be < P when MPS allocates proportionally)

4. MPS (Mobile Power Source)
   - Moves to de-energized areas to temporarily supply power to loads
   - State: idle -> moving -> idle -> connected -> out_of_service
   - After connection, must specify output power and target_loads via set_output()
   - Supports multi-MPS cooperative power supply (power allocated proportionally)

5. DP (Damage Point)
   - Two types: node fault / edge fault (line fault)
   - Node fault: Affects all electrical edges connected to the node
   - Edge fault: Inserts a fault node in the traffic graph, affects corresponding electrical edge
   - capacity: Maximum number of parallel repair crews (MILP constraint)
"""
...
class Switch:
    ...
    def open(self, egraph: 'EGraph') -> None:
        ...
        Args:
            egraph: Electrical graph object, used to update edge connectivity state
        """
        self.state = "open"
    ...
    def operate(self, egraph: 'EGraph', action: str) -> Dict[str, str]:
        ...
        Returns:
            Operation result dictionary with result and reason fields
        """
        if action == "open":
            self.open(egraph)
    ...
    def get_by_node(cls, node_id: str) -> List['Switch']:
        """Get all switches on a given node

        Args:
            node_id: Node ID

        Returns:
            List of all switches on this node
        """
...
class RC:
    ...
    def __init__(self, rc_id: str, initial_position: str, speed: int,
        ...
        Args:
            ...
            efficiency: Max repair efficiency per time step (p.u.)
        ...
        self.auto_repair = False  # Whether to auto-continue repair
        self.last_repair_target = None  # Last repair target fault ID
    ...
    def add(cls, rc_id: str, initial_position: str, speed: int,
        ...
        Args:
            ...
            efficiency: Max repair efficiency
    ...
    def move(self, target_node: str, tgraph: 'TGraph') -> bool:
        ...
        Returns:
            Whether movement was successfully initiated
        """
        # Check state
        ...
        # Cleanup repair state immediately when interrupted
    ...
    def repair(self, dp_id: str, repair_efficiency: Optional[float] = None,
        ...
        Args:
            ...
            repair_efficiency: Repair efficiency for this step (p.u./time step)
                             None means use max efficiency
            auto_repair: Whether to auto-continue repair
        ...
        if self.remaining_resources <= 0:
            ...
            self.state = "out_of_service"  # Out of service after resources depleted
            return 0
        ...
        # If this RC is not in active_crews, check if capacity is exceeded
        ...
        if repair_efficiency is None:
            repair_efficiency = self.efficiency  # Use max efficiency
        else:
            repair_efficiency = min(repair_efficiency, self.efficiency)  # Cannot exceed max

        # Save planned efficiency (for auto-repair)
        self._repaired_this_step = True
        self.planned_efficiency = repair_efficiency

        # Consider resource limit
    ...
    def set_out_of_service(self):
        """Manually set RC to out of service"""
        # CRITICAL: Remove from active_crews before going out of service
        if self.last_repair_target and self._agent_manager:
            dp = self._agent_manager.get_damage_point(self.last_repair_target)
            if dp:
        ...
        if self.state == "repairing":
            ...
            if self.auto_repair and self.last_repair_target and tgraph and egraph:
                if not self._repaired_this_step:
                    # Use saved efficiency to continue repair
    ...
    def _at_fault_location(self, dp: 'DP') -> bool:
        ...
        # Edge fault - RC must reach the exact fault position (fault_node_id) to repair
...
class Load:
    ...
    def __init__(self, load_id: str, node_id: str, P: int, Q: int, W: int, initial_state: str = "off"):
        ...
        # Cumulative power supply (retained after normal MPS disconnect, but reset to 0 when MPS power insufficient)
        # Used to implement partial power retention: MPS supplied 3kW then normally disconnected, load retains 3kW
        # Note: When MPS power is insufficient (insufficient_power/no_output), P_retained is reset to 0
        # Load's "restored" state is guaranteed by AgentManager._restored_loads monotonic flag
        self.P_retained = 0.0  # Current locked active power supply (kW)
        self.Q_retained = 0.0  # Current locked reactive power supply (kVar)
    ...
    def switch_off(self, force_system: bool = False) -> None:
        """Switch load off (de-energize / load shedding)

        Args:
            force_system: True means system-forced shedding, bypasses user constraint
                         False means manual user operation, checks constraint

        Note:
            Reliability constraint applies to manual operations.
            force_system=True allows system-driven shedding.
        """
    ...
    def get_by_node(cls, node_id: str) -> List['Load']:
        """Get all loads on a given node

        Args:
            node_id: Node ID

        Returns:
            List of all loads on this node
        """
...
class MPS:
    ...
    def __init__(self, mps_id: str, p_limit: int, s_limit: int, energy: int,
        ...
        self.initial_energy = energy  # Store initial energy for reference

        # Movement parameters
        self.speed = speed  # km/h - Movement speed for each MPS
    ...
    def add(cls, mps_id: str, p_limit: int, s_limit: int, energy: int,
        ...
        # Create MPS instance (passing speed parameter)
    ...
    def update_position(self) -> None:
        ...
        # Decrease remaining distance (using each MPS's own speed)
        self.remaining_distance -= self.speed
    ...
    def connect(self, egraph: 'EGraph', target_loads: List[str] = None) -> bool:
        """Connect to electrical node at current position and specify loads to serve
        ...
        # First add MPS node to electrical graph (if not present)
        from .graphs import NodeType
        ...
        # Initial output is 0, awaiting explicit setting via set_output() by LLM
        self.Pout = 0
        self.Qout = 0
    ...
    def update_energy(self) -> None:
        ...
        # Use declared output directly for energy consumption
        # update_system_state() handles proper power allocation separately
        consumption = self.Pout
    ...
    def set_output(self, P_out: float, Q_out: float, target_loads: List[str], egraph: 'EGraph' = None) -> Dict[str, Any]:
        """Set MPS output power (LLM action) - v5.1 simplified

        If MPS is not connected, automatically connects to current position.
        ...
        Returns:
            Result dictionary with status, message, and constraint info
        """
        import math

        # 0. Integer conversion - force output to integer, simplify LLM learning (avoid float precision issues)
        ...
        # Note: P_max and S_max capacity enforcement is already done above (after rounding).
        # P_out is clamped to p_limit, and S = sqrt(P^2+Q^2) is scaled to s_limit if exceeded.
    ...
    def calculate_output(self, island_loads: List) -> Tuple[int, int]:
        """Calculate MPS actual output (to be implemented)
    ...
    def update_all(cls, tgraph: 'TGraph' = None) -> None:
        """Update all MPS states (called each time step)

        Args:
            tgraph: Traffic graph object (for position updates)
        """
...
class DP:
    ...
    def __init__(self, dp_id: str, from_node: str, to_node: str, distance: int, repair_demand: int, capacity: int = 1):
        ...
        Args:
            ...
            to_node: End node (if equal to from_node, represents a node fault)
        ...
        self.split_edges = []  # Edge IDs after splitting in traffic graph
        self.affected_node_edges = []  # Edges affected by node fault
    ...
    def _apply_node_fault(self, node_id: str, tgraph: 'TGraph', egraph: 'EGraph'):
        ...
        """
        # Use TGraph's method to mark node fault
    ...
    def _apply_edge_fault(self, from_node: str, to_node: str, distance: int,
        ...
        if original_edge:
            ...
            print(f"WARNING: DP {self.id} could not find edge {from_node}-{to_node} in traffic graph")
    ...
    def repair(self, amount: float, tgraph: 'TGraph', egraph: 'EGraph') -> float:
        ...
        if self.repair_progress >= self.repair_demand:
            ...
            # Clear active_crews (all RCs leave after repair completion)
    ...
    def complete_repair(self, tgraph: 'TGraph', egraph: 'EGraph'):
        """Post-repair completion processing
        ...
        if self.from_node == self.to_node and self.distance == 0:
            ...
            # Edge fault repair - fault node marked as non-fault (but node is retained)
    ...
    def check_edge_affected(cls, edge_id: str, egraph: 'EGraph') -> bool:
        ...
        Returns:
            True if affected by fault and cannot conduct
        """
