"""
Independent DSR Environment - Instance-based environment management
Works with AgentManager for true parallel training
"""
...
class IndependentDSREnvironment:
    ...
    def update_switch_effects(self, allow_faulted: bool = False, revert_on_cycle: bool = True) -> Dict[str, Any]:
    ...
    def check_power_balance(self) -> Dict:
        ...
        try:
            ...
            supply_snapshot = {}
            if hasattr(self.egraph, 'build_supply_snapshot'):
                supply_snapshot = self.egraph.build_supply_snapshot()

            # Calculate total connected MPS generation
            total_mps_generation = 0
            for mps in mps_sources.values():
                if mps.state == 'connected':
                    total_mps_generation += mps.Pout

            # Grid supply is exogenous in this simulator, so only compare MPS generation
            # against the served demand actually supplied by MPS.
            total_grid_served = 0.0
            total_mps_served = 0.0
            for load_id, load in loads.items():
                info = supply_snapshot.get(load_id, {}) if isinstance(supply_snapshot, dict) else {}
                served_grid = float(info.get('served_grid_kw', 0.0) or 0.0)
                served_mps = float(info.get('served_mps_kw', 0.0) or 0.0)

                # Fallback for older snapshots: infer Grid-vs-MPS supply from state.
                if not info and load.state == "on":
                    served_mps = min(
                        float(load.P),
                        max(0.0, float(getattr(load, 'P_retained', 0.0) or 0.0)),
                    )

                total_grid_served += served_grid
                total_mps_served += served_mps
    ...
    def _auto_resolve_repair_cycle(self) -> Optional[str]:
        ...
        causes the least disruption (fewest isolated loads). This is a system-level
        auto-resolution — no penalty should be applied.
        ...
        if not cycle_details or not cycle_details.get('switches_in_cycle'):
            # Fallback: try all closed switches to find one that breaks the cycle
            candidates = [
        ...
        # Try each candidate: open it, check if cycle is resolved
    ...
    def _calculate_weighted_power(self) -> float:
        ...
        Returns sum of (W * P) for all instantaneously served loads.
