"""
Agent Manager - Instance-based agent state management
...
class AgentManager:
    ...
    def add_load(self, load_id: str, node_id: str, P: float, Q: float, W: float) -> Load:
        ...
        Note: Load.state is an internal monotonic restoration marker:
        - state="on": load has been restored at least once in this episode
        - state="off": load has never been restored yet
        - Instantaneous serving must use currently_served/served_grid_kw/served_mps_kw
          from get_current_state() (topology + power-balance aware)

        Load state management (AUTOMATIC):
        - Loads start as "off" and are switched on automatically when supply is feasible
        - Once restored, loads are not forcibly switched off (monotonic restoration)
        - System restoration is handled in graphs.py:update_system_state()
        - LLM controls RC repair and MPS deployment; load restoration is automatic
        - The operate_load() interface exists for compatibility but is not exposed to LLM
        """
        load = Load(load_id, node_id, P, Q, W, initial_state="off")
    ...
    def export_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'mobile_power_sources': {mps_id: {
                ...
                'steps_at_current_node': mps.steps_at_current_node  # Stay constraint counter
    ...
    def import_state(self, state: Dict[str, Any]) -> None:
        ...
        for mps_id, mps_data in state.get('mobile_power_sources', {}).items():
            ...
            # Restore stay constraint counter
            mps.steps_at_current_node = mps_data.get('steps_at_current_node', 1)
